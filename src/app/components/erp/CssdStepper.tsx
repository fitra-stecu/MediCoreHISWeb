...
const CSSD_STEPS = [
  ...
  { step: 4, nama: "Sterilisasi", desc: "Autoklaf uap jenuh 134°C / EO Gas untuk instrumen sensitif panas", icon: Thermometer, durasi: "60 mnt" },
...
export default function ERPCSSD() {
  ...
  const handleCreate = (data: Omit<Batch, "id" | "status">) => {
    ...
    close();
    toast(`Batch ${newBatch.id} berhasil diinput — ${data.instrumen}`);
  };
  ...
  const handleUpdate = (data: Omit<Batch, "id" | "status">) => {
    ...
    close();
    toast(`Batch ${selected.id} berhasil diperbarui`);
  };
  ...
  const handleDelete = () => {
    ...
    close();
    toast(`Batch ${id} berhasil dihapus`, "danger");
  };

  // ── Refresh: simulasi perubahan status batch ──
  const handleRefresh = () => {
    setRefreshing(true);
    setTimeout(() => {
      setBatches((prev) =>
        prev.map((b) => {
          // Batch yang belum selesai, maju 1 step
          if (b.step < 5) {
            const nextStep = b.step + 1;
            return { ...b, step: nextStep, status: STEP_STATUS_MAP[nextStep] || b.status };
          }
          // Batch yang sudah di step 5, tandai selesai (ini simulasi saja)
          if (b.step === 5 && b.status !== "selesai") {
      ...
      setRefreshing(false);
      toast("Data batch diperbarui", "info");
    }, 1500);
  };

  // ── Filter by step ──
  const filteredBatches = activeStep === 0
    ? batches
    : batches.filter((b) => b.step === activeStep);

  // ── Hitung ringkasan ──
  const totalInstrumen = batches.reduce((s, b) => s + b.qty, 0);
  const prosesCount = batches.filter((b) => b.step < 5).length;
  const selesaiCount = batches.filter((b) => b.step >= 5).length;
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-border flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold text-foreground font-mono">{prosesCount}</div>
            <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Sedang Proses</div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border border-border flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold text-foreground font-mono">{selesaiCount}</div>
      ...
      <div className="bg-white rounded-xl border border-border p-5 shadow-sm">
        ...
        <div className="flex items-start gap-0 overflow-x-auto pb-2">
          {/* "Semua" tab */}
          <div
            className="flex flex-col items-center gap-2 cursor-pointer w-28 shrink-0 text-center p-3 rounded-xl transition-all border-2 border-transparent hover:bg-muted/50"
          ...
          {CSSD_STEPS.map((s, i) => {
            ...
            return (
              <div key={i} className="flex items-center">
                <div
                  className={`flex flex-col items-center gap-2 cursor-pointer w-28 shrink-0 text-center p-3 rounded-xl transition-all border-2 ${isActive ? "bg-primary/10 border-primary" : isDone ? "opacity-70 border-transparent hover:bg-muted/50" : "opacity-50 border-transparent hover:bg-muted/50"}`}
                ...
                >
                  ...
                  {count > 0 && (
                    <span className="inline-flex items-center justify-center px-1.5 py-0.5 bg-primary/10 text-primary text-[9px] font-bold rounded-md mt-0.5">
                      {count}
                    </span>
                  )}
                </div>
                {i < CSSD_STEPS.length - 1 && (
        ...
        <div className="mt-4 p-3 bg-muted rounded-lg text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">{activeStep === 0 ? "Menampilkan semua batch" : CSSD_STEPS[activeStep - 1].nama}:</span>{" "}
          {CSSD_STEPS[activeStep === 0 ? "Klik tahap di atas untuk filter" : CSSD_STEPS[activeStep - 1].desc]}
        </div>
      </div>

      {/* ═══ TABEL BATCH ═══ */}
      <SectionCard
        title={`Batch Sterilisasi${activeStep > 0 ? ` — Tahap ${CSSD_STEPS[activeStep - 1].nama}` : ""}`}
      ...
      >
        {filteredBatches.length === 0 ? (
          <div className="text-center py-12 text-sm text-muted-foreground">Tidak ada batch di tahap ini</div>
        ) : (
          <TableWrapper>
            <thead>
              <tr>
                ...
                <tr key={b.id} className="hover:bg-muted/40 transition-colors">
                  ...
                  <Td>
                    <div className="flex items-center gap-2">
                      <ProgressBar value={b.step} max={5} color={C.green} />
